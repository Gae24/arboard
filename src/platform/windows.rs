...
use std::{borrow::Cow, marker::PhantomData, ptr, thread, time::Duration};
...
impl<'clipboard> Get<'clipboard> {
	...
	pub(crate) fn text(self) -> Result<String, Error> {
		...
		if unsafe { IsClipboardFormatAvailable(CF_UNICODETEXT.into()) == 0 } {
			return Err(Error::ContentNotAvailable);
		}

		let text_size = clipboard_win::raw::size(CF_UNICODETEXT.into())
		...
		let bytes_read = {
			...
			let mut bytes_read = clipboard_win::raw::get(CF_UNICODETEXT.into(), out)
